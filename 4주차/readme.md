...
직접 loss function을 구성하게 되면 항상 최소화가 아니라 최대화도 할 때가 있음
sgd, adam 등등 loss를 가정하고 짜는 알고리즘 즉, 기울기를 뺌  
앞에 -만 붙여주면 최대화  
optimizer.zero.grad()
...
다하는건아니고
업스트림 다운스트림 메시지
돌아가기 위한 가능한 모든 미분값의 곱셈의 합산

토치의 텐서 정의  
옵티마이저 정의  
파이토치는 내부적으로 다 생성이 됌
안만들고싶으면 no_grad것 안에서 컴피티션을 만들어준다.
텐서플로우는 그건 아니고 만들어주고 정의할 필요가 있음
gradient 함수업데이트 api 차이?]==
